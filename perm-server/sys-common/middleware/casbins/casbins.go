...
func CheckPermissions(ctx context.Context) bool {
	...
	//uid := strconv.Itoa(string(user.Id))
	uid := strconv.Itoa(int(0))
	fmt.Println(user)
	fmt.Println(uid)
	yes := true //GetEnforcer().Enforce(uid, ctx.Path(), ctx.Method(), ".*", "")
	if !yes {
		supports.Unauthorized(ctx, supports.PermissionsLess, nil)
		ctx.StopExecution()
		return false
	}
	ctx.Params().Set(commonConst.CURRENT_USER_KEY, user.Id)
	return true
	//ctx.Next()
}
