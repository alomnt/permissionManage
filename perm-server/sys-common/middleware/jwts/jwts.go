...
func Serve(ctx context.Context) bool {
	...
	if err := jwts.CheckJWT(ctx); err != nil {
		//supports.Unauthorized(ctx, supports.TokenParseFailurAndInvalid, nil)
		ctx.StopExecution()
		golog.Errorf("token验证：Check jwt error, %s", err)
		return false
	}
	return true
...
// 解析token的信息为当前用户
func ParseToken(ctx context.Context) (*user.User, bool) {
	mapClaims := (jwts.Get(ctx).Claims).(jwt.MapClaims)

	id, ok1 := mapClaims["id"].(string)
	userAccount, ok2 := mapClaims["userAccount"].(string)
	userPassword, ok3 := mapClaims["userPassword"].(string)
...
// 在登录成功的时候生成token
func GenerateToken(user *user.User, currentUser *baseModel.CurrentUser) (string, error) {
	...
	// token放缓存
	caches.Set("token_"+user.Id, token)
	// 当前用户缓存
	caches.Set("currentUser_"+user.Id, utils.StructToString(currentUser))
	return token, err
}
